...
### Added
- **Design deep-dive + dashboard.** `docs/design.md` narrates the decisions, tradeoffs, and failure reasoning end to end; `deploy/grafana/outbox-dashboard.json` is an importable Grafana dashboard for the `signalium.outbox.*` metrics (datasource templated; wiring documented in `docs/observability.md`).
